...
const GetnerateWordCloud = ({
  ...
  if (!maxFontSize) {
    if (frequencies.length == 1) {
      ...
      if (ret.length >= 2) {
        fontSize = ~~(
          (2 * ret[0].fontSize * ret[1].fontSize) /
          (ret[0].fontSize + ret[1].fontSize)
        );
  ...
  for (const e of frequencies) {
    ...
    fontSize = ~~(
      (relativeScaling * (e.value / lastFreq) + (1 - relativeScaling)) *
      fontSize
    );
    ...
    while (true) {
      ...
      if (pos != null) {
        textPosition = {
          ...
          textX:
            (pos % width) + margin / 2 - (!orientation ? 0 : fontSize * 0.055),
          textY:
            ~~(pos / width) +
            margin / 2 +
            (!orientation ? fontSize * 0.055 : 0),
        };
        break;
      }
    ...
    const offCtx = new OffscreenCanvas(
      textPosition.sizeX,
      textPosition.sizeY,
    ).getContext('2d');
