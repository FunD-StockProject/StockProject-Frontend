...
export const fetchStockPreference = (stockId: number) => fetchAuthData(`/preference/stock/${stockId}`);
...
export const postHide = (stockId: number | string) => fetchAuthData(`/preference/hide/${stockId}`, { method: 'POST' });
...
export const fetchBookmarkList = (): Promise<FavoriteStock[]> => fetchAuthData(`/preference/bookmark/list`);

// GET /preference/bookmark/count
export const fetchBookmarkCount = (): Promise<number> => fetchAuthData(`/preference/bookmark/count`);
